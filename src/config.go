...
func parseFlags() {
	...
	// Default the DuckDB temp directory so a memory_limit can spill to disk instead of
	// erroring. Harmless when no limit is set (DuckDB only spills under memory pressure).
	if _config.DuckDbTempDirectory == "" {
		_config.DuckDbTempDirectory = os.TempDir()
	}
	if _config.LogLevel == "" {
